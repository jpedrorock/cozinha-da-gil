...
### 2026-05-25
- [claude-pastel 2026-05-25 background] **E2E Playwright: fluxo completo de pedido** — `e2e/order-flow.spec.ts` (3 cenários: fluxo completo atendente→cozinha→entregue, transição inválida PEDIDO_FEITO→PRONTO, atendente bloqueado de cancelar em EM_PREPARO). Fix: `api-smoke.spec.ts` usava senha antiga de Gil (1234 → 2699). Nota: `auth.spec.ts` (browser UI) exige `npx playwright install` — pré-existente no env cloud.
