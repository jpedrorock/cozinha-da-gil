...
_Vazio — nenhum item em andamento._
...
### 2026-06-14
- [claude-pastel 2026-06-14 background] **[P3] Investigar routine background** — causa raiz identificada: branch criada no passo 2 do prompt (antes do loop de triagem), gerando branches log-only quando não há itens elegíveis. Documentado em PLAYBOOK → seção "Rotina background — evitar branches log-only". Fix recomendado: triagem antes de `git checkout -b`. Enquanto não corrigido: se routine não achar trabalho real, commitar STATUS direto em main sem criar branch.
- [claude-pastel 2026-06-14 background] **[P3] Botão WhatsApp sinaliza link incluído** — `ComprovanteClient.tsx` ganhou micro-texto abaixo do botão "Enviar no WhatsApp": "Mensagem inclui link de acompanhamento" (com `LinkIcon` 11px), visível só quando `trackUrl` está disponível (publicToken presente + baseUrl montado). Atendente não precisa copiar o link separado.
- [claude-pastel 2026-06-14 background] **[P3] Bookkeeping pós-merge Fase 7** — `docs/FASE-7.md`: banner atualizado para "✅ ENTREGUE (5/5)", features #3/4/5 marcadas como ✅ FEITA com referência aos PRs. `BACKLOG.md`: "Em progresso" zerado (PR #4 e Next 15 já mergeados — removidos).
