...
### 2026-05-25
- [claude-pastel 2026-05-25 background] Fix `$queryRawUnsafe` pra PRAGMA journal_mode = WAL em `lib/prisma.ts` — elimina erro cosmético "Execute returned results" nos logs de scripts CLI
- [claude-pastel 2026-05-25 background] Doc `docs/UPGRADE-NEXT-15.md` — plano de upgrade Next 14→15 com breaking changes, 19 handlers afetados, estimativa 7–9h, rollout recomendado
- [claude-pastel 2026-05-25 background] E2E `e2e/order-flow.spec.ts` — fluxo completo atendente→cozinha→entregue com sessões separadas por role; e2e inconclusivo nesta sessão (sem servidor em execução, mesmo status dos testes existentes)
