...
_Sem itens em andamento no momento._
...
- [x] **[P3] #atendente #ux** Botão "Enviar WhatsApp" do comprovante sinaliza que link já tá incluído [claude-pastel 2026-06-14 background]
...
- [x] **[P3] #chore #docs** Bookkeeping pós-merge de Fase 7 [claude-pastel 2026-06-14 background]
...
### 2026-06-14
- [claude-pastel 2026-06-14 background] **[P3] Investigar routine background criando branches sem trabalho** — causa raiz identificada: o prompt da Routine cria branch `routine-pastel-*` no Passo 2 ANTES de varrer o BACKLOG por itens executáveis. Se todos os itens são "Confirmar antes"/P0/SSE, a rotina abre branch vazia de qualquer forma. Fix documentado em `PLAYBOOK.md` → "Problema conhecido: rotina criando branches mesmo sem trabalho". João precisa atualizar prompt da Routine no Cowork: verificar itens executáveis PRIMEIRO; se nenhum, atualizar STATUS em main diretamente sem abrir branch.
- [claude-pastel 2026-06-14 background] **[P3] Botão WhatsApp sinaliza link incluído** — `ComprovanteClient.tsx`: micro-texto "Mensagem inclui link de acompanhamento" (ícone `LinkIcon` 12px) aparece abaixo do botão "Enviar no WhatsApp" quando `trackUrl` está disponível (pedido tem `publicToken`). Atendente vê que o link já vai na mensagem e não tenta copiar/mandar separado. lint + testes verdes.
- [claude-pastel 2026-06-14 background] **[P3] Bookkeeping pós-merge de Fase 7** — `docs/FASE-7.md`: todos os 5 itens marcados ✅ (PR #23 Next 15, #28 troco, #29 comparativo, #31 WhatsApp, #5 "Acabou"); header do doc atualizado com "Status: ✅ ENTREGUE". `BACKLOG.md`: seção "Em progresso" zerada (PR #4 backup + PR Next 15 ambos mergeados em 09/06). `STATUS.md` atualizado com estado atual.
