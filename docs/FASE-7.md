...
- **Entregue:** sub-aba "Comparativo" no admin Vendas — cards por `EventSession` com faturamento, nº pedidos, ticket médio, hora de pico e topping campeão do evento. Comparação lado a lado. Mergeada em 2026-05-29.
...
- **Entregue:** banner fixed-bottom 1-toque quando pedido vira PRONTO e cliente tem telefone. Reusa `notifyReady()` existente. Mergeada em 2026-06-01.
...
- **Verificado em 2026-05-29:** toggle do admin já propagava via SSE (`ingredient:updated`) → atendente reflete `available` ao vivo sem refresh. Nenhum trabalho necessário.
