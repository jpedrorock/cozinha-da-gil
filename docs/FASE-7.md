...
> **Status: ✅ ENTREGUE (5/5 — 2026-06-05).** Este doc é referência histórica da fase. Todas as
> features foram implementadas e mergeadas em main. O código é a fonte de verdade.
...
- **Entregue:** sub-aba "Comparativo" em admin Vendas — cards por `EventSession` com faturamento, nº pedidos, ticket médio, topping campeão. Reusa agregação existente. Sem schema.
...
- **Entregue:** banner fixed-bottom 1-toque no atendente quando pedido vira PRONTO e tem telefone cadastrado. Reusa `notifyReady()` existente. Só dispara com telefone.
...
## 5. "Acabou" — propagação ao vivo (admin marca) — ✅ FEITA (verificada 2026-05-29)

- **Entregue:** toggle admin propaga ao vivo via SSE `ingredient:updated` → atendente aplica `available` em tempo real. Verificado que já estava implementado.
