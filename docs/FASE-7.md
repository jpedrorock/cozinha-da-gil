...
> **Status: ✅ ENTREGUE (5/5).** Todas as features foram construídas e mergeadas — detalhes abaixo.
...
- **Entregue:** sub-aba "Comparativo" no admin Vendas. Card por `EventSession` com faturamento, nº pedidos, ticket médio. Reusa agregação existente. Sem schema novo.
...
- **Entregue:** banner fixed-bottom no atendente que aparece automaticamente quando pedido vira PRONTO e tem telefone cadastrado. 1 toque manda a mensagem prefillada (`notifyReady()`). Follow-up: botão Avisar auto-volta pra fila após o toque (routine 2026-06-05).
...
- **Verificado (2026-05-29):** `PATCH /api/ingredients/[id]` → broadcast `ingredient:updated` SSE → atendente aplica `available` ao vivo (chip fica cinza sem refresh). Já estava implementado; nenhum código novo necessário.
