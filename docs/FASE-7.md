...
- **Entregue:** sub-aba "Comparativo" no admin Vendas; card por `EventSession` (faturamento, nº pedidos, ticket médio, hora de pico, topping campeão). Squash-merged PR #29 em main (2026-05-29).
- **Como:** read-only, reusa a agregação existente. Sem schema. Endpoint estático novo.

## 4. WhatsApp "tá pronto" — auto-surface 1 toque — ✅ FEITA (PR #31)

- **Entregue:** banner fixed-bottom 1-toque no atendente quando pedido vira PRONTO + tem telefone. Reusa `notifyReady()` existente. Squash-merged PR #31 em main (2026-06-01).
...
- **Verificado 2026-05-29:** toggle "esgotou" do admin já propaga ao vivo via `ingredient:updated` SSE → atendente aplica `available` em tempo real sem refresh. Feature estava pronta desde Fase 6.
