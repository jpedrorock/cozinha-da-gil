...
> **Status: ✅ ENTREGUE (5/5 features implementadas).** Este doc é o plano original mantido como
> referência histórica. Todas as 5 features foram construídas e mergeadas ao main. PR #4 (backup
> automático do `dev.db`) ainda aguarda review/merge — feature independente das 5 abaixo.
> **Como:** cada feature entrou por PR própria.
...
- **Entregue:** nova sub-aba "Comparativo" no admin → Vendas: cards por `EventSession` com faturamento, nº pedidos e ticket médio; coluna de detalhe mostra top toppings e hora de pico por evento. Read-only, sem schema novo. Mergeado junto com troco (PR #28) e migração Next 15 (PR #23).
...
- **Entregue:** banner fixed-bottom aparece automaticamente no atendente quando pedido vira PRONTO e tem telefone cadastrado. 1 toque abre `wa.me` com mensagem de "tá pronto" prefillada (reusa `notifyReady()` + template existente). Bannerem tem 2 ações: "Avisar" (abre WhatsApp) e "Dispensar". Cobre tanto card do atendente quanto banner de notificação direta.
...
## 5. "Acabou" — propagação ao vivo (admin marca) — ✅ FEITA (verificado 2026-05-29, já existia)

- **Entregue:** verificado que já funcionava — `PATCH /api/ingredients/[id]` (toggle `available`) dispara broadcast SSE `ingredient:updated`; atendente recebe e aplica `available` ao vivo no chip do ingrediente. Nenhum código novo necessário — a feature já estava completa.
...
- **Verificação:** PATCH ingrediente → broadcast `ingredient:updated` → atendente aplica `available` ao vivo. Feature já existia, nada a fazer.
