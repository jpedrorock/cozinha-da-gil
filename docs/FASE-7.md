...
> **Status: ✅ ENTREGUE.** Este doc é o **plano original** da Fase 7, mantido como referência histórica. Todas as 5 features foram implementadas e mergeadas no main (PIX PR #30, Troco PR #28, Comparativo PR #29, WhatsApp PR #31, Acabou já existia). Next 15 em prod desde 2026-05-29. O código é a fonte de verdade.
...
## 3. Comparativo entre eventos (admin) — ✅ FEITA (PR `claude-pastel/comparativo-eventos`)

- **Entregue:** sub-aba "Comparativo" no admin Vendas com cards por `EventSession` (faturamento, nº pedidos, ticket médio, hora de pico, topping campeão). Lado a lado, read-only. Mergeada como PR #29.
...
- **Entregue:** banner fixed-bottom 1-toque na tela do atendente quando pedido vira PRONTO e cliente tem telefone. Reusa `notifyReady()` + template `templateOrderReady`. Mergeada como PR #31.
...
- **Verificado 2026-05-29:** toggle "esgotou" do admin já propagava via SSE `ingredient:updated` para o atendente (chip cinza imediato, sem refresh). Nada a fazer.
