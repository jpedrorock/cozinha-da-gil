...
> **Status: ✅ ENTREGUE.** Todas as 5 features implementadas e mergeadas em main (2026-05-29 → 2026-06-09).
> PR #28 (troco), #29 (comparativo), #23 (Next 15), #30 (PIX), #31 (WhatsApp pronto), #4 (backup) — todos squash-merged.
> O código é a fonte de verdade. Este doc é referência histórica das decisões travadas.
...
- **Verificado 2026-05-29:** PATCH ingrediente já emite broadcast `ingredient:updated` → atendente aplica `available` ao vivo via SSE. Feature existia antes da Fase 7.
- **Fora de escopo:** envio 100% sem toque exigiria WhatsApp Business + Cloud API (conta paga + template aprovado) — overkill pra barraca.
