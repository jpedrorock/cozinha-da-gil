...
> **Status: ✅ ENTREGUE.** Todas as 5 features foram construídas e mergeadas no main.
> PIX (PR #30 / squash `12f205f`), Troco (PR #28), Comparativo (PR #29), WhatsApp auto-surface
> (PR #31), "Acabou" (já existia no main). Next 15 + @ducanh2912/next-pwa mergeado via PR #23.
> Coolify deployou automaticamente. Este doc é referência histórica — o código é a fonte de verdade.
...
- **Entregue:** sub-aba "Comparativo" em Vendas mostra card por EventSession com faturamento, pedidos, ticket médio, hora de pico e topping campeão. Mergeado em 2026-05-29.
...
- **Entregue:** banner fixed-bottom aparece ao marcar PRONTO quando cliente tem telefone. Reusa `notifyReady()`. Mergeado em 2026-06-01.
...
- **Verificado:** toggle "esgotou" do admin já propagava ao vivo via SSE (`ingredient:updated`) → atendente reflete na hora. Nada a construir.
